...
export default async function DashboardHome({ searchParams }: { searchParams: Promise<{ paid?: string; session_id?: string }> }) {
  ...
  // Past = any 'accepted' signup whose session has now happened. We used to
  // also require the host to have published the final list, but publishing is
  // an email-notification action that's orthogonal to "did the user attend" —
  // an accepted attendee is a record of attendance regardless.
  ...
  return (
    <div className="p-4 sm:p-8 max-w-5xl">
      ...
      {/* PAST EVENTS — sessions the user was accepted for that have now happened. */}
      <section className="mb-8">
        <h2 className="text-lg font-semibold mb-4" style={{ fontFamily: 'var(--font-display)' }}>Past events</h2>
        {pastEvents.length === 0 ? (
          <p className="text-sm" style={{ color: 'var(--color-muted)' }}>
            Sessions you were accepted for will show up here once they&apos;ve happened.
          </p>
        ) : (
          <div className="grid gap-3">
      ...
      <div className="flex flex-col items-end gap-1 flex-shrink-0">
        {cancelled ? (
          ...
          <Badge tone={{ bg: '#FFF4B8', fg: '#7C5800' }}>
            Sign-ups open {new Date(opensAtIso).toLocaleString('en-GB', { weekday: 'long', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit', timeZone: 'Europe/London' })}
          </Badge>
        ) : past ? null : (
          <AttendRequestButton
            eventId={event.id}
